...
export class Path extends AbstractShape {
        ...
        case 'a':
          if (params.length >= 7) {
            ...
            for (const curve of curves) {
              path.addCubicCurve(curve.cp1x, curve.cp1y, curve.cp2x, curve.cp2y, curve.x, curve.y);
            }
  ...
  private static approximateArc(
    ...
    for (let i = 0; i < segments; i++) {
      ...
      const alpha =
        (Math.sin(segmentSweep) *
          (Math.sqrt(4 + 3 * Math.tan(segmentSweep / 2) * Math.tan(segmentSweep / 2)) - 1)) /
        3;

      // 현재 세그먼트의 시작점
      const startX =
        cx + rx * Math.cos(segmentAngle) * cosAngle - ry * Math.sin(segmentAngle) * sinAngle;
      const startY =
        cy + rx * Math.cos(segmentAngle) * sinAngle + ry * Math.sin(segmentAngle) * cosAngle;

      // 현재 세그먼트의 끝점
      const endX =
        cx +
        rx * Math.cos(nextSegmentAngle) * cosAngle -
        ry * Math.sin(nextSegmentAngle) * sinAngle;
      const endY =
        cy +
        rx * Math.cos(nextSegmentAngle) * sinAngle +
        ry * Math.sin(nextSegmentAngle) * cosAngle;

      // 제어점 계산
      const cp1x =
        startX +
        alpha * (-rx * Math.sin(segmentAngle) * cosAngle - ry * Math.cos(segmentAngle) * sinAngle);
      const cp1y =
        startY +
        alpha * (-rx * Math.sin(segmentAngle) * sinAngle + ry * Math.cos(segmentAngle) * cosAngle);

      const cp2x =
        endX -
        alpha *
          (-rx * Math.sin(nextSegmentAngle) * cosAngle -
            ry * Math.cos(nextSegmentAngle) * sinAngle);
      const cp2y =
        endY -
        alpha *
          (-rx * Math.sin(nextSegmentAngle) * sinAngle +
            ry * Math.cos(nextSegmentAngle) * cosAngle);
   ...
   */
  private findQuadraticExtrema(p0: Point2D, p1: Point2D, p2: Point2D): number[] {
    const result: number[] = [];
   ...
   * @returns 극점의 t 매개변수 값 배열 (0과 1 사이)
   */
  private findCubicExtrema(p0: Point2D, p1: Point2D, p2: Point2D, p3: Point2D): number[] {
    const result: number[] = [];
   ...
   * @returns 곡선 상의 점
   */
  private evaluateQuadraticBezier(p0: Point2D, p1: Point2D, p2: Point2D, t: number): Point2D {
    const mt = 1 - t;
    return {
  ...
  get bounds(): Bounds {
    ...
    for (let i = 1; i < this._points.length; i++) {
      ...
      if (currentPoint.type === 'quadratic' && currentPoint.controlPoint) {
        ...
        for (const t of extremaParams) {
          transformedPoints.push(
            this.evaluateQuadraticBezier(prevTransformed, controlTransformed, currentTransformed, t)
          );
        }
      } else if (
        currentPoint.type === 'cubic' &&
